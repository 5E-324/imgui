...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos)
{
    ...
    if (hovered) {
        ...
        ImVec2 half_size = ImVec2(g.FontSize * 0.5f, g.FontSize * 0.5f);
        window->DrawList->AddRectFilled(center - half_size, center + half_size, col);
    }

    float cross_extent = g.FontSize * 0.4f * 0.7071f;
    float thickness = g.Style.LineThickness;//g.FontSize * 0x1p-5;
    ImU32 cross_col = GetColorU32(ImGuiCol_Text);
    center -= ImVec2(0.5f, 0.5f);
    window->DrawList->AddLine(center + ImVec2(+cross_extent, +cross_extent), center + ImVec2(-cross_extent, -cross_extent), cross_col, thickness);
    window->DrawList->AddLine(center + ImVec2(+cross_extent, -cross_extent), center + ImVec2(-cross_extent, +cross_extent), cross_col, thickness);

    return pressed;
}

bool ImGui::CloseButton(ImGuiID id, const ImRect &bb)
{
    ImGuiContext &g = *GImGui;
    ImGuiWindow *window = g.CurrentWindow;
    ...
    float cross_extent = g.FontSize * 0.4f * 0.7071f;
    float thickness = g.Style.LineThickness;//g.FontSize * 0.05f;
    ImU32 cross_col = GetColorU32(ImGuiCol_Text);
    center -= ImVec2(0.5f, 0.5f);
    window->DrawList->AddLine(center + ImVec2(+cross_extent, +cross_extent), center + ImVec2(-cross_extent, -cross_extent), cross_col, thickness);
    window->DrawList->AddLine(center + ImVec2(+cross_extent, -cross_extent), center + ImVec2(-cross_extent, +cross_extent), cross_col, thickness);
...
bool ImGui::CollapseButton(ImGuiID id, const ImVec2& pos, ImGuiDockNode* dock_node)
{
    ...
    ImVec2 padding = ImVec2(g.Style.TitleBarPadding.y, g.Style.TitleBarPadding.y);
    ...
    if (dock_node)
        ...
        RenderArrow(window->DrawList, bb.Min + padding, text_col, window->Collapsed ? ImGuiDir_Down : ImGuiDir_Up, 1.0f);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    WidgetLayout(&total_bb, &bb, &label_bb, w, label, label_end);
    ItemSize(total_bb, style.FramePadding.y);
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    WidgetLayout(&total_bb, &frame_bb, &label_bb, w, label, label_end);
    ...
    if (temp_input_is_active)
    {
        ...
        frame_bb.TranslateX(total_bb.Min.x - frame_bb.Min.x);
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    if ((flags & ImGuiTreeNodeFlags_SpanFullWidth) || display_frame)
    {
        frame_bb.Min.x -= IM_FLOOR(window->WindowPadding.x);
        frame_bb.Max.x += IM_FLOOR(window->WindowPadding.x);
    }
    ...
    const float text_offset_x = g.FontSize + padding.x * 2;//(display_frame ? padding.x * 3 : padding.x * 2);           // Collapser arrow width + Spacing
    const float text_offset_y = ImMax(padding.y, window->DC.CurrLineTextBaseOffset);                    // Latch before ItemSize changes it
    const float text_width = g.FontSize + (label_size.x > 0.0f ? label_size.x + padding.x * 2 : 0.0f);  // Include collapser
    ImVec2 text_pos(window->DC.CursorPos.x + text_offset_x, window->DC.CursorPos.y + text_offset_y);
    //float temp = g.Style.ItemSpacing.y;
    //g.Style.ItemSpacing.y = 0;
    ItemSize(ImVec2(text_width, frame_height), padding.y, true);
    ...
    else
    {
        ...
        if (flags & ImGuiTreeNodeFlags_Bullet)
            ...
            RenderArrow(window->DrawList, ImVec2(text_pos.x - text_offset_x + padding.x, text_pos.y), text_col, is_open ? ((flags & ImGuiTreeNodeFlags_UpsideDownArrow) ? ImGuiDir_Up : ImGuiDir_Down) : ImGuiDir_Right, 0x0.Dp0f);
        if (g.LogEnabled)
            LogSetNextTextDecoration(">", NULL);
...
bool    ImGui::BeginTabBarEx(ImGuiTabBar* tab_bar, const ImRect& tab_bar_bb, ImGuiTabBarFlags flags, ImGuiDockNode* dock_node)
{
    ...
    tab_bar->FramePadding = (dock_node != NULL) ? g.Style.TitleBarPadding : g.Style.FramePadding;
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar)
{
    ...
    sections[0].Spacing = 0.0f;//sections[0].TabCount > 0 && (sections[1].TabCount + sections[2].TabCount) > 0 ? g.Style.ItemInnerSpacing.x : 0.0f;
    sections[1].Spacing = 0.0f;//sections[1].TabCount > 0 && sections[2].TabCount > 0 ? g.Style.ItemInnerSpacing.x : 0.0f;
    ...
    for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
    {
        ...
        tab->ContentWidth = (tab->RequestedWidth >= 0.0f) ? tab->RequestedWidth : TabItemCalcSize(tab_name, has_close_button_or_unsaved_marker, tab_bar->FramePadding).x;

        int section_n = TabItemGetSectionIdx(tab);
        ImGuiTabBarSection* section = &sections[section_n];
        section->Width += tab->ContentWidth + 0.0f;//(section_n == curr_section_n ? g.Style.ItemInnerSpacing.x : 0.0f);
    ...
    for (int section_n = 0; section_n < 3; section_n++)
    {
        ...
        for (int tab_n = 0; tab_n < section->TabCount; tab_n++)
        {
            ...
            tab->NameOffset = -1;
            tab_offset += tab->Width + 0.0f;//(tab_n < section->TabCount - 1 ? g.Style.ItemInnerSpacing.x : 0.0f);
        }
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    ImVec2 size = TabItemCalcSize(label, (p_open != NULL) || (flags & ImGuiTabItemFlags_UnsavedDocument), tab_bar->FramePadding);
...
ImVec2 ImGui::TabItemCalcSize(const char* label, bool has_close_button_or_unsaved_marker, ImVec2 padding)
{
    ImGuiContext& g = *GImGui;
    ImVec2 label_size = CalcTextSize(label, NULL, true);
    ImVec2 size = ImVec2(label_size.x + padding.x, label_size.y + padding.y * 2.0f);
    if (has_close_button_or_unsaved_marker)
        size.x += padding.x + (g.Style.ItemInnerSpacing.x + g.FontSize); // We use Y intentionally to fit the close button circle.
    else
        size.x += padding.x + 1.0f;
    return ImVec2(ImMin(size.x, TabBarCalcMaxTabWidth()), size.y);
}

ImVec2 ImGui::TabItemCalcSize(ImGuiWindow* window)
{
    return TabItemCalcSize(window->Name, window->HasCloseButton || (window->Flags & ImGuiWindowFlags_UnsavedDocument), GImGui->Style.TitleBarPadding);
}
